...
class BlockTest {
    ...
    @ParameterizedTest
    @MethodSource
    void constructorTest(List<String> userNames, PollCreateStrategy pollCreateStrategy, boolean evenExpected, boolean oddExpected) {
        Block block = new Block(new InitInfo(userNames, pollCreateStrategy));

        IntStream.rangeClosed(0, userNames.size())
